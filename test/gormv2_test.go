...
// 树形化2
func TestSqlResultToTreeStruct2(t *testing.T) {
	...
	`
	params := "0,1,8,10"
	var res = make([]DataLIst, 0)
	var dest = make([]DataLIst, 0)
	variable.GormDbMysql.Raw(sql, params, params, params).Find(&res)

	if err := sql_res_to_tree.CreateSqlResFormatFactory().ScanToTreeData(res, &dest); err == nil {
